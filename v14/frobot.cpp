...
void geraeixos(Group* group){ 
	...
	ve[idx]->anglemax=100;
	ve[idx]->offset=offset;
	ve[idx]->newdr(vec3(-42.19,271,-42.19),vec3(-50,271,-42.19));
...
int posa(lua_State* L){
	...
	std::thread thm([](vint p, int sz ){
	while(posapool.size()!=0){
	
    // std::unique_lock<std::mutex> lk(cv_m);
	
	// std::thread thm([](vint p, int sz,int counter ){
		// counter=sz;
		posa_mtx.lock();
		// counter=sz;
		...
		cot(p);
		lop(i,0,sz){ ;
			// float newangle= lua_tonumber(L,i+1); 
			// float newangle= p[i]; 
			float newangle= posapool[0][i]; 
			
			// std::thread th([&](float newangle, int i ){				
		...
		while(posa_counter!=0){cot(posa_counter);sleepms(100);}
		// mtxunlock(10);
