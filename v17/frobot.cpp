	...
	vector<float> posik(vec3 topoint){
		...
		for(int wi=0;wi<1000;wi++){
			for(int vi=sz-1;vi>=0;vi--){
				if( ve[vi]->angleik >  ve[vi]->anglemax || ve[vi]->angleik <  ve[vi]->anglemin ){ 
					cdir[vi]*=-1;
		...
		while(posapool.size()!=0){ 
			...
			lop(i,0,sz){ ;  
				float newangle= posapool[0][i];
  				if(newangle>=1000){posa_counter--; continue;}
...
void posi(int index,float angle){
	...
	if(posapool.size()>0)
		lop(i,0,ve.size())angles[i]=posapool.back()[i];
	else
...
// https://dis.dankook.ac.kr/lectures/med08/wp-content/uploads/sites/35/1/1321343577.pdf 
	...
	fle=new FlEditor(0,90,150,400-60);
	flt->resizable(fle);   
	flt->end();
