...
struct osgdr{
	...
	void rotatesmooth(float rangle,int _dir){
		targetangle=rangle;
				// cot(targetangle);
		currangle=angle;
		dir=_dir;
		threadDetach([&](){
			for(;;){
				...
				cot(angle);
				cot(anglemax);
				if(angle>anglemax) targetangle=0;
				if(angle<anglemin) targetangle=0;
				rotate( targetangle*dir);
				// cot(currangle);
				// cot(targetangle);
				if(currangle>= targetangle || currangle<= targetangle)break;
			}
		});
		// if(anglemax
	...
	void rotate( float _angle ){
		...
		// mt->addChild( loadedModel.get()  );osg::Matrix R; 
		lop(i,0,index){
			osg::Matrix Tr;
		...
		lop(i,index+1,ve.size()){
				// ve[i]->rotate(axis,_angle,*axisbegin,1);
...
// cot(text); 
	...
	for ( int idx=0;idx<fparsenew.size();idx++ ){  
		...
		ifc("pos"){
			...
			lop(i,0,s.size()){
				int vi=vindex[i];
				float rangle=abs(vrangle[i]);
				int dir=vrangle[i]>0?1:-1;
				cot(dir);
				ve[vi]->rotatesmooth(rangle,dir); 
			}
		}
		ifc("posa"){
			string fp=fparsenew[idx][1];
...
// https://dis.dankook.ac.kr/lectures/med08/wp-content/uploads/sites/35/1/1321343577.pdf 
	...
	Fl_Button* bt2=new Fl_Button(30,  0, 30, 30,"bugrot");
