...
struct osgdr{
	...
	void rotateik_dep( float _angle ){ //deprecated
	...
	vector<float> posik(vec3 topoint){
		...
		vfloat angles(sz);
		vec3 axisb=*axisbegin;
		#define cdist distance_two_points(&topoint,axisend)
		...
		float ld=0;
		vfloat cdir(sz);
		lop(i,0,sz)cdir[i]=0.1;
		cd=cdist;
		ld=cd;
		for(int wi=0;wi<100;wi++){
			for(int vi=sz-1;vi>=0;vi--){
				ve[vi]->rotate(cdir[vi]);
				if(ld<=cdist){
					cdir[vi]*=-1;
					ve[vi]->rotate(cdir[vi]);
					// rotate(cdir[vi]);
			...
			ld=distance_two_points(&topoint,axisend);
			cot(ld);
			cot(cdir);
		}
	...
	lop(vei,0,ve.size()){
		...
		// cot(to_string((int)abs(btn/10)));
			...
			btp[vei][i]->callback([](Fl_Widget *, void* v){ 
				...
				ve[vv->index]->rotate_posk(vv->angle);
			},(void*)vsend);
			btn+=10;
		}
	}
